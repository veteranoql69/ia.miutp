...
import {
  ...
  ChevronDown, X, CheckCircle2, Clock, BarChart2, Maximize2
} from 'lucide-react'
import SimceResultsTable from './SimceResultsTable'

// ── Modal fullscreen de un ensayo ─────────────────────────────────────────────

function MaximizedEnsayoModal({ ensayo, userId, onClose }: {
  ensayo: any
  userId?: string
  onClose: () => void
}) {
  ...
  return (
    ...
    >
      {/* Header del modal */}
      <div className="flex items-center justify-between px-8 py-5 border-b border-slate-800/60 bg-slate-950/80 backdrop-blur-xl shrink-0">
        <div>
          <h2 className="text-base font-bold text-slate-100 truncate max-w-2xl">
        ...
        <button
          onClick={onClose}
          className="p-2.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 border border-slate-800/60 hover:border-slate-700 text-slate-400 hover:text-slate-200 transition-all cursor-pointer"
          title="Cerrar (Esc)"
        >
          <X className="w-4 h-4" />
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6">
        <EnsayoDetailPanel
          ensayoId={ensayo.id}
          nRespuestas={ensayo.n_respuestas ?? 0}
          userId={userId}
          onClose={onClose}
        />
      </div>
...
function EnsayoDetailPanel({ ensayoId, nRespuestas, userId, onClose }: {
  ensayoId: string
  nRespuestas: number
  userId?: string
  onClose: () => void
  ...
  const [activeSection, setActiveSection] = useState<'config' | 'resultados'>(
    nRespuestas > 0 ? 'resultados' : 'config'
  )
  ...
  return (
    <div className="mt-4 rounded-2xl border border-slate-700/60 bg-slate-900/80 backdrop-blur-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-800/60">
        <div className="flex gap-1">
          ...
          >
            ...
            Resultados
            {nRespuestas > 0 && (
              <span className="px-1.5 py-0.5 rounded-full bg-emerald-950/60 border border-emerald-800/50 text-emerald-400 text-[8px] font-black">
                {nRespuestas}
              </span>
            )}
          </button>
      ...
      {activeSection === 'resultados' && (
        <div className="p-5">
          {nRespuestas > 0 && userId ? (
            <SimceResultsTable ensayoId={ensayoId} userId={userId} />
          ) : (
            <div className="py-10 text-center space-y-3">
              <Clock className="w-8 h-8 text-amber-500/50 mx-auto" />
              <p className="text-sm font-semibold text-amber-300">Sin hojas de respuesta procesadas</p>
              <p className="text-[11px] text-slate-500 max-w-xs mx-auto">
                Sube y procesa las hojas de tus alumnos en{' '}
                <span className="text-indigo-400">Centro de Herramientas → Gestión de Hojas</span>
                {' '}para ver los resultados por alumno.
              </p>
            </div>
          )}
        </div>
            ...
            <div className="p-5 max-h-[65vh] overflow-y-auto space-y-5">
              ...
              <div className={`flex items-center gap-3 p-3 rounded-xl border ${
                ...
                <p className="text-xs text-slate-300">
                  {detail.n_respuestas > 0
                    ? `${detail.n_respuestas} respuestas de alumnos procesadas · ver tab Resultados`
...
function EnsayoSimceCard({ ensayo, userId }: {
  ensayo: any
  userId?: string
}) {
    ...
    <>
      ...
      >
        <div
          ...
          title="Doble clic para expandir · botón ⤢ para pantalla completa"
        >
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button
                ...
                className="p-1.5 rounded-lg hover:bg-indigo-950/60 hover:border hover:border-indigo-800/50 text-slate-500 hover:text-indigo-400 transition-all"
        ...
        {expandido && (
          <div className="px-5 pb-5">
            <EnsayoDetailPanel ensayoId={ensayo.id} nRespuestas={ensayo.n_respuestas ?? 0} userId={userId} onClose={() => setExpandido(false)} />
          </div>
        )}
      </div>
    </>
...
export default function BentoGridKPIs({ ensayoSeleccionado, colegioId, userId }: BentoGridKPIsProps) {
  ...
  useEffect(() => {
    if (!colegioId) { setLoadingEnsayos(false); return }
    getEnsayosSimce(colegioId).then(res => {
